...
jQuery(document).ready(function ($) {
    ...
    var parentWptmDbtable;
    $(document).on('change', '#wptm_mysql_tables', function (e) {
        parentWptmDbtable = $('#mainTabContent').contents('.db_table.active');
        ...
        $.ajax({
            ...
            if (result.response === true) {
                ...
                parentWptmDbtable.find('#wptm_mysql_tables_columns').html('');
                for (i = 0; i < availableTableColumns.all_columns.length; i++) {
                    col = availableTableColumns.all_columns[i];
                    parentWptmDbtable.find('#wptm_mysql_tables_columns').append('<option value="' + col + '" >' + col + '</option>');
                }

                parentWptmDbtable.find('.wptm_define_mysql_relations').find('div.mysqlRelationsContainer').html('');
                parentWptmDbtable.find('.wptm_define_mysql_relations').hide();
                if (selected_tables.length > 1) {
                    ...
                    for (var i in availableTableColumns.sorted_columns) {
                        ...
                        var mysqlRelationBlockTemplate = parentWptmDbtable.find('#wptm-template-mysqlRelationBlock').html();
                        var template = Handlebars.compile(mysqlRelationBlockTemplate);
                        var relationBlockHtml = template(mysql_table_block);

                        parentWptmDbtable.find('.wptm_define_mysql_relations').find('div.mysqlRelationsContainer').append(relationBlockHtml);

                    }
                    parentWptmDbtable.find('.wptm_define_mysql_relations').show();
                }
            } else {
    ...
    $(document).on('change', '#wptm_mysql_tables_columns', function (e) {
        ...
        parentWptmDbtable.find('.column_title').css('display','none');
        old_default_ordering = parentWptmDbtable.find('#wptm_mysql_default_ordering_column').val();
        parentWptmDbtable.find('#wptm_mysql_default_ordering_column').empty();
        ...
        if(constructedTableData.columnCount) {
            for(i=0;i< constructedTableData.columnCount; i++ ) {
                ...
                if(parentWptmDbtable.find('#wptm_column_' + column_id).length === 0 ) {
                    parentWptmDbtable.find('.columnsTitleContainer').append('<div class="wptm_row column_title" id="wptm_row_'+ column_id+'"><label>'+ constructedTableData.mysql_columns[i] +' custom title: </label><input type="text" name="" id="wptm_column_'+ column_id+'" class="" value=""  /></div>');
                }
                parentWptmDbtable.find('#wptm_row_' + column_id).css('display','block');

                parentWptmDbtable.find('#wptm_mysql_default_ordering_column').append('<option value="'+constructedTableData.mysql_columns[i]+'" >'+constructedTableData.mysql_columns[i]+'</option>') ;
            }

            parentWptmDbtable.find('#wptm_mysql_default_ordering_column').val(old_default_ordering);
        }
    ...
    $(document).on('click', '#wptm_mysql_add_where_condition', function (e) {
        ...
        var whereBlockTemplate = parentWptmDbtable.find('#whereConditionTemplate').html();
        var template = Handlebars.compile(whereBlockTemplate);

        var where_block = {post_type_columns: parentWptmDbtable.find('#wptm_mysql_tables_columns option').map(function () {
                return $(this).val();
            }).toArray()};
        var whereBlockHtml = template(where_block);
        parentWptmDbtable.find('.wptm_define_mysql_conditions').find('div.mysqlConditionsContainer').append(whereBlockHtml);
    ...
    $(document).on('click', '#wptm_mysql_add_grouping_rule', function (e) {
        ...
        grouping_rule_block.post_type_columns = parentWptmDbtable.find('#wptm_mysql_tables_columns').val();

        var groupingRuleBlockTemplate = parentWptmDbtable.find('#groupingRuleTemplate').html();
        var template = Handlebars.compile(groupingRuleBlockTemplate);
        var groupingRuleHtml = template(grouping_rule_block);

        parentWptmDbtable.find('.wptm_define_mysql_grouping').find('div.mysqlGroupingContainer').append(groupingRuleHtml);
    });
    /**
    ...
    $(document).on('click', '#btn_preview', function (e) {
        ...
        for(i = 0; i<constructedTableData.mysql_columns.length; i++) {
            column_id = constructedTableData.mysql_columns[i].replace(".","_");
            if(parentWptmDbtable.find('#wptm_column_'+ column_id).val() ) {
                constructedTableData.custom_titles.push( parentWptmDbtable.find('#wptm_column_'+ column_id).val() );
            }else {
                constructedTableData.custom_titles.push( constructedTableData.mysql_columns[i]);
            }            
        }
       
        constructedTableData.default_ordering = parentWptmDbtable.find('#wptm_mysql_default_ordering_column').val();
        constructedTableData.default_ordering_dir = parentWptmDbtable.find('#wptm_mysql_default_ordering_dir').val();
        constructedTableData.enable_pagination = parentWptmDbtable.find('#wptm_mysql_table_pagination').val();
        constructedTableData.limit_rows = parentWptmDbtable.find('#wptm_mysql_number_of_rows').val();
        
        /**
         * Join rules
         */
        parentWptmDbtable.find('.mysqlRelationsContainer').find('.mysql_table_blocks').each(function(){
            var join_rule = {};
         ...
         * Where block
         */
        parentWptmDbtable.find('.mysqlConditionsContainer').find('.post_where_blocks').each(function(){
            var where_condition = {};
         ...
         * Grouping rules
         */
        parentWptmDbtable.find('.mysqlGroupingContainer').find('.post_grouping_rule_blocks select').each(function(){
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.response) {
                    ...
                    parentWptmDbtable.find('.wptm_previewTable').html(data.preview);
                    parentWptmDbtable.find('#btn_tableCreate').prop('disabled', false);
                    parentWptmDbtable.find('#btn_tableUpdate').prop('disabled', false);
                }
            }
        })
    });
