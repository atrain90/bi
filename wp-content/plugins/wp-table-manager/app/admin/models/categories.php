...
class WptmModelCategories extends Model
{
    ...
    {
        global $wpdb;
        $result = $wpdb->query('SELECT c.* FROM ' . $wpdb->prefix . 'wptm_categories as c WHERE c.level >0 ORDER BY c.lft ASC');
        if ($result === false) {
            return false;
        }
        return stripslashes_deep($wpdb->get_results('SELECT c.* FROM ' . $wpdb->prefix . 'wptm_categories as c WHERE c.level >0 ORDER BY c.lft ASC', OBJECT));
    }
