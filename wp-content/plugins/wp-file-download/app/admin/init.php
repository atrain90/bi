...
function wpfd_menu()
{
    ...
    add_submenu_page(
        'wpfd',
        'Download statistics',
        esc_html__('Download statistics', 'wpfd'),
        'manage_options',
        'wpfd-statistics',
...
//    wp_enqueue_style('wpfd-bootstrap', plugins_url('assets/css/bootstrap.min.css', __FILE__), array(), $ver);
    ...
    wp_enqueue_style('wpfd-calendar', plugins_url('assets/css/calendar-jos.css', __FILE__), array(), $ver);
    wp_enqueue_script('wpfd-calendar', plugins_url('assets/js/calendar.js', __FILE__), array(), $ver);
    wp_enqueue_script('wpfd-calendar-setup', plugins_url('assets/js/calendar-setup.js', __FILE__), array(), $ver);
    wp_enqueue_style(
        'wpfd-datetimepicker',
...
function wpfd_calendartranslation()
{
    ...
    $weekdays_full = array(
        esc_html__('Sunday', 'wpfd'),
        esc_html__('Monday', 'wpfd'),
        esc_html__('Tuesday', 'wpfd'),
        esc_html__('Wednesday', 'wpfd'),
        esc_html__('Thursday', 'wpfd'),
        esc_html__('Friday', 'wpfd'),
        esc_html__('Saturday', 'wpfd'),
        esc_html__('Sunday', 'wpfd')
    );
    ...
    $today = ' ' . esc_html__('Today', 'wpfd') . ' ';
    $text = array(
        'INFO' => esc_html__('About the Calendar', 'wpfd'),
        'ABOUT' => '',
        'ABOUT_TIME' => "\n\n"
            . "Time selection:\n"
        ...
        'PREV_YEAR' => esc_html__('Select to move to the previous year. Select and hold for a list of years.', 'wpfd'),
        'PREV_MONTH' => esc_html__('Select to move to the next month. Select and hold for a list of the months.', 'wpfd'),
        'GO_TODAY' => esc_html__('Go to today', 'wpfd'),
        'NEXT_MONTH' => esc_html__('Select to move to the next month. Select and hold for a list of the months.', 'wpfd'),
        'SEL_DATE' => esc_html__('Select a date.', 'wpfd'),
        'DRAG_TO_MOVE' => esc_html__('Drag to move.', 'wpfd'),
        'PART_TODAY' => $today,
        'DAY_FIRST' => esc_html__('Display %s first', 'wpfd'),
        'WEEKEND' => '0,6',
        'CLOSE' => esc_html__('Close', 'wpfd'),
        'TODAY' => esc_html__('Today', 'wpfd'),
        'TIME_PART' => esc_html__('(Shift-)Select or Drag to change the value.', 'wpfd'),
        'DEF_DATE_FORMAT' => '%Y-%m-%d',
        'TT_DATE_FORMAT' => esc_html__('%a, %1$b %2$e', 'wpfd'),
        'WK' => esc_html__('wk', 'wpfd'),
        'TIME' => esc_html__('Time:', 'wpfd')
    );
...
function wpfd_menu_highlight()
{
    ...
    if ((string) $submenu_file === 'edit-tags.php?taxonomy=wpfd-tag') {
        $parent_file = 'wpfd'; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited -- Force bold it
    }
}
...
function wpfd_button($context)
{
    ...
    $context .= "<span class='dashicons dashicons-download' style='line-height: inherit;'>";
...
function wpfd_calendar_enqueue()
{
    ...
    if ($page === 'wpfd-statistics') {
        ...
        <script type="text/javascript">
            <?php echo wpfd_calendartranslation(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Nothing need escape?>
        </script>
        <?php
    }
}
