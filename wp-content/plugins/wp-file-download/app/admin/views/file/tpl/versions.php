...
if (!empty($this->versions)) {
    ...
    foreach ($this->versions as $meta_id => $file) {
        ...
        $content .= (int) $this->file_id . '&vid=' . (int) $file['meta_id'] . '&catid=' . (int) $file['catid'] . '" >';
        $content .= date('Y M d', strtotime($file['created_time'])) . ' ';
        $content .= '</a></td>';
        $content .= '<td>' . WpfdHelperFiles::bytesToSize((int) $file['size']) . '</td>';
        $content .= '<td><a data-id="' . (int) $this->file_id . '" data-vid="' . (int) $file['meta_id'] . '" data-catid="';
        ...
        if (apply_filters('wpfdAddonCategoryFrom', $file['catid']) === 'dropbox') {
            ...
            $content .= '<a data-id="' . (int) $this->file_id . '" data-vid="' . (int) $file['meta_id'] . '" data-catid="';
