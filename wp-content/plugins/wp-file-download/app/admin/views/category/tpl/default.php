...
foreach ($wp_roles->roles as $k => $userRole) {
    $selected = '';
    if (in_array(strtolower($k), $this->category->roles)) {
        $selected = 'checked="checked"';
    }
    $options[] = '<input type="checkbox"
     name="params[roles][]" value="' . strtolower($k) . '" ' . $selected . '/>' . esc_html($userRole['name']) . '<br/>';
}
                        ...
                        <select name="params[social]" id="wpfd-social-locker" class="ju-input">
                            <option value="0"
                                <?php echo (isset($this->category->params['social']) &&
                                    $this->category->params['social'] === 0) ? 'selected="selected"' : ''; ?>>
                                <?php esc_html_e('No', 'wpfd'); ?>
                            </option>
                            <option value="1"
                                <?php echo (isset($this->category->params['social']) &&
                                    $this->category->params['social'] === 1) ? 'selected="selected"' : ''; ?>>
