 ...
 * @return string
 */
function wpfd_admin_ui_configuration_build_content($name, $html)
{
 ...
 * @return string
 */
function wpfd_admin_ui_configuration_build_tabs($tabs)
{
...
{
    if ($hook !== 'wp-file-download_page_wpfd-config') {
        return;
    }
    ...
    wp_register_script('wpfd-admin-ui-script-configuration', plugins_url('assets/ui/js/configuration.js', __FILE__), array('jquery'), WPFD_VERSION);
    ...
    // Load fonts
    wp_enqueue_style('wpfd-admin-ui-font-nutiosans', 'https://fonts.googleapis.com/css?family=Nunito+Sans');
    $scripts = array(
