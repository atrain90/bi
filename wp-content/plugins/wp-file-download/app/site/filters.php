...
class WpfdFilter extends Filter
{
    ...
    public function load()
    {
        ...
        add_filter('rewrite_rules_array', array($this, 'wpfdInsertRewriteRules'));
        ...
        if ($this->includeGlobalSearch || $enableFts) {
            ...
            add_filter('the_posts', array($this, 'wpfdGetMeta'), 0);
     ...
     * @return array
     */
    public function wpfdGetMeta($posts)
    {
    ...
    private function wpfdCheckAccess($post, $user)
    {
        ...
        if ((int) $category->access === 1) {
            ...
            if (isset($category->params['canview']) &&
                ...
                is_countable($category->roles) && // phpcs:ignore PHPCompatibility.PHP.NewFunctions.is_countableFound -- is_countable() was declared in functions.php
    ...
    public function callSingleFile($file_id, $catid, $nameDisplay = null)
    {
        ...
        if ($font_color !== '') {
            $singleCss .= '.wpfd-single-file .wpfd-file-link a {color: ' . esc_html($font_color) . ' !important;}';
        }
        if ($hover_color !== '') {
            $singleCss .= '.wpfd-single-file .wpfd-file-link a:hover {background-color: ' . esc_html($hover_color) . ' !important;}';
        }
        wp_add_inline_style('wpfd-front', $singleCss);

        $variables = array(
            'file' => $file,
            'nameDisplay' => !is_null($nameDisplay) ? $nameDisplay : $file->title,
            'showsize' => $showsize
        );
        ...
        if (in_array($ordering, $ordering_array)) {
            switch ($ordering) {
                ...
                case 'ext':
                    ...
                    break;
                case 'version':
                    usort($files, array('WpfdFilter', 'cmpVersion'));
                    break;
                case 'description':
            ...
            $content = do_shortcode(
                '[wpfdasocial]' . $theme->showCategory($options) . ($category->params['theme'] !== 'tree' ?
                    wpfd_category_pagination(
                        array('base' => '', 'format' => '', 'current' => max(1, $page), 'total' => $total)
                    ) : ''
                ) . '[/wpfdasocial]'
            );
        } else {
    ...
    private function contentAllCat($shortcode_param = false)
    {
        ...
        foreach ($categories as $keyCat => $category) {
            $allFile1 = $this->fileAllCat($category->term_id, $shortcode_param);
            if (!empty($allFile1)) {
        ...
        if (in_array($ordering, $ordering_array)) {
            switch ($ordering) {
                ...
                case 'ext':
                    ...
                    break;
                case 'version':
                    usort($allFiles, array('WpfdFilter', 'cmpVersion'));
                    break;
                case 'description':
        ...
        $modelCategory = Model::getInstance('category');
        $category = $modelCategory->getCategory($categories[0]->term_id);
        $categories = array();
        ...
        $themefile = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'themes' . DIRECTORY_SEPARATOR . 'wpfd-default';
        $themefile .= DIRECTORY_SEPARATOR . 'theme.php';
        include_once $themefile;
        $class = 'WpfdTheme' . ucfirst('default');
        $theme = new $class();
        ...
        if (isset($param_number) && $param_number) {
            $files = array_slice($allFiles, 0, $param_number);
        }
        ...
        $options = array('files' => $files, 'category' => $category, 'categories' =>
            $categories, 'params' => $params, 'tpl' => null, 'latest' => true);
        $content = $theme->showCategory($options) . ($category->params['theme'] !== 'tree' ?
                wpfd_category_pagination(
                    array('base' => '', 'format' => '', 'current' => max(1, $page), 'total' => $total)
                ) : ''
     ...
     * @return array
     */
    public function getAllFileRef($model, $listCatRef, $ordering, $orderingDirection)
    {
        $lstAllFile = array();
        foreach ($listCatRef as $key => $value) {
            if (is_array($value) && !empty($value)) {
                $lstFile    = $model->getFiles($key, $ordering, $orderingDirection, $value);
    ...
    {
        return $this->compareByProperty($a, $b, 'version', 'string');
    }
