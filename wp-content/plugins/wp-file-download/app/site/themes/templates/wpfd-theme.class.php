...
class WpfdTheme
{
    ...
    public function loadAssets()
    {
        ...
        wp_enqueue_script(
            'handlebars',
            plugins_url('app/site/assets/js/handlebars-1.0.0-rc.3.js', WPFD_PLUGIN_FILE),
            array(),
            WPFD_VERSION
        );
        wp_enqueue_script(
    ...
    public static function outputContentWrapper($theme)
    {
        ...
        // Print hidden input
        $html = sprintf(
            '<input type="hidden" id="current_category" value="%s" />
                    <input type="hidden" id="current_category_slug" value="%s" />',
    ...
    public static function outputContentHeader($theme)
    {
        ...
        if ((int) WpfdBase::loadValue($theme->params, self::$prefix . 'showbreadcrumb', 1) === 1 && !$theme->latest) {
            ...
            if ($showDownloadAll && !$theme->categoryFrom) {
                $output .= sprintf(
                    ...
                    esc_attr('display-download-category'),
                    esc_url($theme->category->linkdownload_cat),
                    esc_html__('Download all ', 'wpfd')
                );
            }
            $output .= '<ul class="head-category-' . self::$themeName . '"><li>&nbsp;</li></ul>';
        }
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- this escaped
        echo $output;
    }
    ...
    public static function showTitleHandlebars($config, $params)
    {
        if ((int) WpfdBase::loadValue($params, self::$prefix . 'showtitle', 1) === 1) {
            $name     = self::$themeName;
            $template = array(
                'html' => '<h3><a href="%url$s" class="wpfd_downloadlink" title="%title$s">%text$s</a></h3>',
                'args' => array(
                    'url'   => '{{linkdownload}}',
                    'title' => '{{post_title}}',
                    'text'  => '{{crop_title}}'
    ...
    public static function showDownloadHandlebars($config, $params)
    {
        if ((int) WpfdBase::loadValue($params, self::$prefix . 'showdownload', 1) === 1) {
            ...
            $template = array(
                'html' => '<a class="%class$s" href="%url$s" style="%style$s">%text$s%icon$s</a>',
                'args' => array(
                    'class' => 'downloadlink wpfd_downloadlink',
                    'url'   => '{{linkdownload}}',
                    'style' => $style,
                    'text'  => esc_html__('Download', 'wpfd'),
                    'icon'  => '<i class="zmdi zmdi-cloud-download wpfd-download"></i>'
                )
            );
            /**
    ...
    public static function showCategoryTitleHandlebars($theme, $params)
    {
        $name     = self::$themeName;
        $template = array(
            'html' => '<a class="catlink wpfdcategory backcategory" href="#" data-idcat="{{parent}}">
                        %icon$s</i>%text$s</a>',
            'args' => array(
    ...
    public static function showCategoriesHandlebars($theme, $params)
    {
        ...
        $output .= self::render('{{#if categories}}{{#each categories}}' . $args['html'] . '{{/each}}{{/if}}', $args['args']);
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- this escaped
        echo $output;
    }
    ...
    {
        $style = 'margin : ' . WpfdBase::loadValue($params, self::$prefix . 'margintop', 5) . 'px ';
        $style .= WpfdBase::loadValue($params, self::$prefix . 'marginright', 5) . 'px ';
        $style .= WpfdBase::loadValue($params, self::$prefix . 'marginbottom', 5) . 'px ';
        $style .= WpfdBase::loadValue($params, self::$prefix . 'marginleft', 5) . 'px;';
